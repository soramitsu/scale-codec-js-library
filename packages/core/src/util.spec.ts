...
describe('Walker', () => {
  ...
  describe('static encode()', () => {
    ...
    it('When allocated array size does not match to actual used bytes, throws', () => {
      ...
      expect(() => Walker.encode(VALUE, ENCODE)).toThrowError(/offset \(3\) is not equal to array bytes length \(5\)/)
    })
  })
  ...
  describe('static decode()', () => {
    ...
    it('When decoded bytes count is not equal to input length, throws', () => {
      ...
      expect(() => Walker.decode(INPUT, DECODE)).toThrowError(/offset \(1\) is not equal to array bytes length \(10\)/)
    })
  })
})
