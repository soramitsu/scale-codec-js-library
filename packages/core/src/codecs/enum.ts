...
export function encodeEnumSizeHint<E extends Enum<any>>(
    ...
    if (encodeData === null)
        throw new EnumEncodeError(
            `Invalid encode schema for Enum with tag "${tag}": ${(encoders as any)[tag]}`,
            encoders,
        )
...
export function decodeEnum<E extends Enum<any>>(walker: Walker, decoders: EnumDecoders<EnumDef<E>>): E {
    ...
    if (!decoder)
        throw new Error(
            `Decode data for discriminant ${discriminant} is undefined; ` +
                `decoders schema: ${formatDecoders(decoders)}`,
        )
