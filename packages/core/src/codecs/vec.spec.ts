...
import { Option, variant } from '@scale-codec/enum'
...
describe.concurrent('Vec codec', () => {
  describe('Vec<T>', () => {
    ...
    it('vec of option int encoded as expected', () => {
      const VEC: Option<number>[] = [variant('Some', 1), variant('Some', -1), variant('None')]
    ...
    it('vec of option bool encoded as expected', () => {
      const VEC: Option<boolean>[] = [variant('Some', true), variant('Some', false), variant('None')]
