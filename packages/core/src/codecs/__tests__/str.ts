import { hexifyBytes, prettyHexToBytes } from '@scale-codec/util'
...
describe('encodeStr', () => {
    it.each(RUST_STR_SAMPLES)('can encode $src', ({ src, hex_scale }) => {
        expect(hexifyBytes(WalkerImpl.encode(src, encodeStr))).toEqual(hex_scale)
    })
})

describe('decodeStr', () => {
    it.each(RUST_STR_SAMPLES)('can decode to $src', ({ src, hex_scale }) => {
        expect(WalkerImpl.decode(prettyHexToBytes(hex_scale), decodeStr)).toEqual(src)
    })
