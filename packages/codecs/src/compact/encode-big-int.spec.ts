...
describe('encodeBigIntCompact', (): void => {
    it('encodes short u8', (): void => {
        expect(encodeBigIntCompact(JSBI.BigInt(18))).toEqual(new Uint8Array([18 << 2]));
    });

    it('encodes max u8 values', (): void => {
        expect(encodeBigIntCompact(JSBI.BigInt(63))).toEqual(new Uint8Array([0b11111100]));
    });

    it('encodes basic u16 value', (): void => {
        expect(encodeBigIntCompact(JSBI.BigInt(511))).toEqual(new Uint8Array([0b11111101, 0b00000111]));
    });

    it('encodes basic u16 (not at edge)', (): void => {
        expect(encodeBigIntCompact(JSBI.BigInt(111))).toEqual(new Uint8Array([0xbd, 0x01]));
    });

    it('encodes basic u32 values (< 2^30)', (): void => {
        expect(encodeBigIntCompact(JSBI.BigInt(0x1fff))).toEqual(new Uint8Array([253, 127]));
    });

    it('encodes basic u32 values (short)', (): void => {
        expect(encodeBigIntCompact(JSBI.BigInt(0xffff))).toEqual(new Uint8Array([254, 255, 3, 0]));
    });

    it('encodes basic u32 values (full)', (): void => {
        expect(encodeBigIntCompact(JSBI.BigInt(0xfffffff9))).toEqual(
            new Uint8Array([3 + ((4 - 4) << 2), 249, 255, 255, 255]),
        );
    });

    it('encodes a large value', (): void => {
        expect(encodeBigIntCompact(JSBI.BigInt('0x00005af3107a4000'))).toEqual(
    ...
    it('does not modify the original', (): void => {
        ...
        expect(encodeBigIntCompact(original)).toEqual(new Uint8Array([2, 137, 7, 0]));
    ...
    describe('from Rust', (): void => {
        it.each([
            ...
            expect(encodeBigIntCompact(value)).toEqual(
